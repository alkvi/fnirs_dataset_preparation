...
def get_start_quaternion(q_orig, direction, stable_sample = 500, plot_quaternions=False):
    ...
    if plot_quaternions:
        ...
        ff_lf = [0]
        ff_rf = [0]
        plot_quaternions(ax1, q_orig, ff_lf, "Original quaternion")
        plot_quaternions(ax2, q0, ff_rf, "Rotated quaternion")
        plot_euler(ax3, q_orig, ff_lf, "Original euler")
        plot_euler(ax4, q0, ff_rf, "Rotated euler")
        plt.show()
...
if __name__ == "__main__":
    ...
    all_subject_frame.to_csv("temp_data/calibration_stance_data.csv", index=False)
    print('done')